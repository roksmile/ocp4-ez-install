...
```bash
sudo bash 01.connected/01_download_ocp_tools.sh
```
...
```bash
bash 01.connected/02_create_isc.sh
```
...
```bash
bash 01.connected/03_mirror_images.sh
```
...
```bash
sudo bash 02.air-gapped/01_install_tools.sh
```
...
```bash
bash 02.air-gapped/02_create_certs.sh
```
...
```bash
sudo bash 02.air-gapped/03_create_registry.sh
```
...
```bash
bash 02.air-gapped/04_upload_mirror.sh
```
...
```bash
bash 02.air-gapped/05_create_install_config.sh
```
...
```bash
bash 02.air-gapped/06_create_agent_config.sh
```
...
```bash
bash 02.air-gapped/07_create_config_yaml.sh
```
...
```bash
bash 02.air-gapped/08_create_cluster_manifests.sh
```
...
```bash
bash 02.air-gapped/09_create_agent_iso.sh
```
...
```bash
bash 02.air-gapped/10_monitor_install.sh
```
...
bash 03.add-nodes/01_create_nodes_config.sh

# Step 2. 노드 추가 ISO 생성 (oc login 필요)
bash 03.add-nodes/02_create_nodes_iso.sh
```
...
```bash
bash 04.add-operators/01_create_add_operators_isc.sh
```
...
```bash
# RUN_ID 직접 지정
bash 04.add-operators/02_mirror_add_operators.sh 2026032001

# 또는 목록에서 선택
bash 04.add-operators/02_mirror_add_operators.sh
```
